import { JsonContext, LionWebJsonNode, LionWebJsonReference } from "@lionweb/validation"
...
export class ReferenceAdded extends ReferenceChange {
    ...
    protected msg = () => `Node "${this.node.id}: reference with key ${this.reference.reference.key} is added with value ${this.reference.targets.map(t => t.reference)}`
}

export class ReferenceRemoved extends ReferenceChange {
    readonly id = "ReferenceRemoved"
    protected msg = () => `Node "${this.node.id}: reference with key ${this.reference.reference.key} is removed, old value was ${this.reference.targets.map(t => t.reference)}`
}
